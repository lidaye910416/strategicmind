...
class CompanyAwareSimulation:
    ...
    async def _update_metrics(
        ...
        # 基础值
        base_revenue = 0.5 + env.sector_growth_rate * 2
        base_profit = bm.margin_baseline
        base_sentiment = env.consumer_sentiment
        
        # 行动影响
        revenue_change = exec_count * 0.05 - block_count * 0.02
        profit_change = compromise_count * 0.02 - exec_count * 0.01
        
        snapshot = {
            "round": round_num,
            "revenue_outlook": max(-1, min(1, base_revenue + revenue_change)),
            "profit_margin_outlook": max(-1, min(1, base_profit + profit_change - 0.5)),
            "market_sentiment": base_sentiment,
            "competitive_position": 0.5 + (exec_count - block_count) * 0.03,
