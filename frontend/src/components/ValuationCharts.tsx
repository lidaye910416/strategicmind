...
export default function ValuationCharts({ projections, baseValue, upsideValue, downsideValue }: Props) {
  return (
    <div>
      <h3 className="text-sm font-semibold text-gray-900 mb-3">Financial Projections</h3>
      <div className="grid grid-cols-3 gap-3 mb-4">
        ...
        <div className="rounded-lg border border-red-200 bg-red-50 p-3">
          <div className="flex items-center gap-1 text-red-700 text-xs">
            <TrendingDown size={14} /> Downside
          </div>
          <div className="text-lg font-bold text-red-900 mt-1">{formatValue(downsideValue)}</div>
        </div>
      </div>
      {projections.length === 0 ? (
        <p className="text-sm text-gray-400 py-4">No projection data</p>
      ) : (
        <ResponsiveContainer width="100%" height={300}>
          <LineChart data={projections}>
            ...
            <Legend />
            <Line type="monotone" dataKey="upside_revenue" stroke="#4caf50" strokeWidth={2} name="Upside" />
            <Line type="monotone" dataKey="base_revenue" stroke="#3b6bff" strokeWidth={2} name="Base" />
            <Line type="monotone" dataKey="downside_revenue" stroke="#f44336" strokeWidth={2} name="Downside" />
          </LineChart>
        </ResponsiveContainer>
      )}
    </div>
